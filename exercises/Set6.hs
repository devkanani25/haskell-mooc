...
instance Ord Country where
  compare x y | show x == show y = EQ
              | show x <= show y = LT
              | otherwise = GT
  x <= y = compare x y /= GT
  min x y | show x <= show y = x
          | otherwise = y
  max x y | show x <= show y = y
          | otherwise = x
------------------------------------------------------------------------------
...
instance Eq a => Eq (List a) where
  Empty == Empty = True
  (LNode x xs) == Empty = False
  Empty == (LNode y ys) = False
...
instance Ord Number where
  compare (Finite x) (Finite y) | x == y = EQ
                                | x <= y = LT
                                | otherwise = GT
...
instance Eq RationalNumber where
  (RationalNumber x y) == (RationalNumber x' y') = x*y' == y*x'
...
simplify (RationalNumber x y) = RationalNumber x' y'
  where c = gcd x y
        x' = x `div` c
        y' = y `div` c
...
instance Num RationalNumber where
  (RationalNumber x1 y1) + (RationalNumber x2 y2) = simplify (RationalNumber x y)
    where x = (x1*y2) + (x2*y1)
          y = y1 * y2
  ...
  abs (RationalNumber x y) = RationalNumber x' y'
    where x' = if x < 0 then x * (-1) else x
          y' = if y < 0 then y * (-1) else y
          
  signum (RationalNumber x y) = RationalNumber x' 1
    where x' = if x < 0 then (-1) else (if x == 0 then 0 else 1)
          
  fromInteger x = RationalNumber x 1
  
  negate (RationalNumber x y) = RationalNumber (0-x) y
...
class Cycle a where
  ...
  stepMany x y = head . reverse $ go x y
    where go 0 y = [y]
          go 1 y = [step y]
          go x y = (step y) : go (x-1) (step y)
