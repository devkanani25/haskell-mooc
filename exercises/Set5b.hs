...
t :: Tree Int
t = (Node 3 (Node 7 Empty Empty) (Node 1 Empty Empty))
------------------------------------------------------------------------------
...
treeMax Empty = 0
treeMax (Node a leftTree rightTree) = max a (max (treeMax leftTree) (treeMax rightTree))
...
allValues condition (Node a leftTree rightTree) | condition a == False = False
                                                | otherwise = allValues condition leftTree && allValues condition rightTree
...
cull val (Node a leftTree rightTree) | a == val = Empty
                                     | otherwise = (Node a (cull val leftTree) (cull val rightTree))
...
isOrdered (Node a leftTree rightTree) | (not $ (allValues (<a) leftTree)) || (not $ (allValues (>a) rightTree)) = False
                                      | otherwise = isOrdered leftTree && isOrdered rightTree
...
walk (x:xs) (Node a leftTree rightTree) | x == StepL = walk xs leftTree
                                        | x == StepR = walk xs rightTree   
...
set _ _ Empty = Empty
set [] val (Node a leftTree rightTree) = Node val leftTree rightTree
set (x:xs) val (Node a leftTree rightTree) | x == StepL = (Node a (set xs val leftTree) rightTree)
                                           | x == StepR = (Node a leftTree (set xs val rightTree))
...
search val t | hasPath val t == False = Nothing
             | otherwise = Just (search' val t)

search' :: Eq a => a -> Tree a -> [Step]
search' val Empty = []
search' val (Node a l r) | val == a = []
                         | hasPath val l = StepL : search' val l
                         | hasPath val r = StepR : search' val r

hasPath :: Eq a => a -> Tree a -> Bool
hasPath val Empty = False
hasPath val (Node a l r)| val == a  = True
                        | otherwise = hasPath val l || hasPath val r 