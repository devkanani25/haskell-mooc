...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b | measure a > measure b = a
                  | otherwise = b
...
firstHalf str = take l str
  where l = length str `div` 2 + (if even (length str) then 0 else 1) 
...
while check update value | not $ check value = value
...
-- if isLeft (f x) then (f x) else (f x)
...
multiApp f gs x = f xs
  where xs = multiApp' gs x 
