...
largestHelper prev (x : xs) | x < prev  = [prev]
                            | x == prev = prev : (x : xs)  
                            | otherwise = (x : xs)
...
foldr _f [] (x:xs) = Just x
_f x (foldr _f [] xs) = Just x
_f x (myHead xs) = Just x
_f = \x headXs = Just x
-}
...
lastHelper x y = if y == Nothing then Just x else y
