...
allEqual [] = True
allEqual xs = if length (nub xs) == 1 then True else False
...
longest :: Ord a => [[a]] -> [a]
longest xs = head (trimList (sortByLength (sort xs)))

trimList :: [[a]] -> [[a]]
trimList xs = filter (\x -> (length x) == l) xs
  where l = length (last xs)
...
incrementKey key [] = []
incrementKey key ((x,y):xs) | x == key  = (x,y+1) : incrementKey key xs
                            | otherwise = (x,y) : incrementKey key xs
...
winner scores player1 player2 | score1 == score2 = player1
...
freqs xs = Map.fromListWith (+) [(x,1) | x <- xs]
...
transfer from to amount bank | Map.notMember from bank = bank
                             ...
                             | otherwise = transfer' to amount (+) (transfer' from amount (-) bank)
  where fromBalance = findBalance from bank

findBalance :: String -> Map.Map String Int -> Int
findBalance from bank =
  case Map.lookup from bank of
    Nothing -> 0
    Just sum -> sum

transfer' :: String -> Int -> (Int -> Int -> Int) -> Map.Map String Int -> Map.Map String Int
transfer' from amount operator bank =
  case Map.lookup from bank of
    Nothing  -> bank                                   
    Just sum -> Map.insert from (operator sum amount) bank
...
swap i j arr = arr // [(i,elem_j),(j,elem_i)]
  where elem_i = arr ! i
        elem_j = arr ! j
...
maxIndex arr = recursiveFind elem assoc
  where elem = last (sort (elems arr))
        assoc = assocs arr

recursiveFind :: (Ix i, Ord a) => a -> [(i,a)] -> i
recursiveFind e ((x,y):xs) | e == y = x
                           | otherwise = recursiveFind e xs